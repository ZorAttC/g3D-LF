...
class ValueMap(BaseMap):
    ...
    def __init__(
        ...
        self._bev_map = np.zeros((size, size), np.float32)

        self.noise_filter = torch.nn.Conv2d(1, 1, (13, 13), padding=(6,6))
        noise_filter_weight = torch.ones(1,1,13,13) / (13.*13.)
    ...
    def update_bev_map(
        ...
        bev_map = self._localize_bev_map(bev_map, tf_camera_to_episodic)
        self._bev_map = self.noise_filter(torch.tensor(bev_map).unsqueeze(0).unsqueeze(0)).squeeze(0).squeeze(0).numpy()
        # Fuse the new data with the existing data
        if not deadlock:
            self._fused_map = self._value_map[...,0] * np.exp(self._bev_map)
    ...
    def sort_waypoints(
        ...
        def get_value(point: np.ndarray) -> Union[float, Tuple[float, ...]]:
            ...
            all_values = [
                ...
                pixel_value_within_radius(self._fused_map, point_px, radius_px)
    ...
    def _localize_bev_map(
        ...
        if PLAYING:
            if yaw > 0:
                ...
                yaw = np.deg2rad(30)
        curr_data = rotate_image(bev_map, -yaw)
