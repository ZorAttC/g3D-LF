...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def __init__(
        ...
        coco_threshold: float = 0.85,
        non_coco_threshold: float = 0.5,
        *args: Any,
        **kwargs: Any,
    ) -> None:
        super().__init__()
    ...
    def act(
        ...
        self.history_deadlock.append(self.deadlock)
        if self.history_deadlock[-10:].count(6) == 10:
            print("Deadlock happens, try out.")
            action_numpy = random.randint(1,2)
            pointnav_action = torch.tensor([action_numpy])

        self.history_actions.append(action_numpy) # !!!!!!!!!!!!!!!!!
    ...
    def _get_object_detections(self, img: np.ndarray) -> ObjectDetections:
        ...
        if any(c in self.hard_target for c in target_classes):
            detections.filter_by_conf(det_conf_threshold-0.1)
        elif any(c in self.super_hard_target for c in target_classes):
            detections.filter_by_conf(det_conf_threshold-0.3)
        elif any(c in self.easy_target for c in target_classes):
            detections.filter_by_conf(det_conf_threshold+0.05)
        else:
