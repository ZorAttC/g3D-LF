...
class ObjectDetections:
    ...
    def merge(self, other_detections):
        self.boxes = torch.cat(self.boxes, other_detections.boxes)
        self.logits = torch.cat(self.logits, other_detections.logits)
        self.phrases.extend(other_detections.phrases)

    def intersection(self, other_detections):
        phrases_list = []
        for i,phrase in enumerate(self.phrases):
            if phrase not in other_detections.phrases:
                self.boxes = torch.cat([self.boxes[:i],self.boxes[i+1:]], dim=0)
                self.logits = torch.cat([self.logits[:i],self.logits[i+1:]], dim=0)
            else:
                phrases_list.append(phrase)
        self.phrases = phrases_list
