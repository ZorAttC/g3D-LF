...
class Feature_Fields(nn.Module):
    ...
    def run_panorama_encode(self, batch_position=None, batch_direction=None, batch_rot=None, batch_trans=None, view_shuffle=False, visualization=False):

        view_num = 12
        batch_panorama_fts = []
        if view_shuffle:
            shuffled_view_id = random.randint(0, 11) # Shuffle the view_id for training
        for view_id in range(view_num):
            ...
            for batch_id in range(self.batch_size):
                if self.mode == 'habitat':
                    ...
                    if view_shuffle:
                        view_heading_angle = ((view_id-shuffled_view_id)*(-math.pi/6) + camera_direction) % (2.*math.pi) # if view_id == shuffled_id, get the forward-facing view
                    else:
                    ...
                    if view_shuffle:
                        pano_angle = (view_id-shuffled_view_id)*(-math.pi/6) # if view_id == shuffled_id, get the forward-facing view
                    else:
        ...
        if view_shuffle:
            return batch_panorama_fts, shuffled_view_id
        else:
