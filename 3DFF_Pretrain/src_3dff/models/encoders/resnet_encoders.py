...
import torchvision
import src_3dff.models.encoders.clip as clip
...
class CLIPEncoder(nn.Module):
    ...
    def forward(self, observations):
        ...
        rgb_observations = observations["rgb"].permute(0, 3, 1, 2)
        rgb_observations = self.rgb_transform(rgb_observations).to(self.device)
        
        view_fts, grid_fts = self.model.encode_image(rgb_observations)

        return view_fts.detach(), grid_fts.detach()