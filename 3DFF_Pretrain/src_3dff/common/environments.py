...
class VLNCEDaggerEnv(habitat.RLEnv):
    ...
    def current_line_dist_to_goal(self):
        init_state = self._env.sim.get_agent_state()
        init_distance = np.sqrt(np.square(init_state.position - self._env.current_episode.goals[0].position).sum())
        #init_distance = self._env.sim.geodesic_distance(
        #    init_state.position, self._env.current_episode.goals[0].position,
        #)
        return init_distance
    ...
    def cand_line_dist_to_goal(self, angle: float, forward: float):
        r'''get resulting distance to goal by executing 
        a candidate action'''
        ...
        #    post_state.position, self._env.current_episode.goals[0].position,
        #)
        post_distance = np.sqrt(np.square(post_state.position - self._env.current_episode.goals[0].position).sum())
        # reset agent state
        sim.set_agent_state(init_state.position, init_state.rotation)
    ...
    def map_step(self, action, vis_info, *args, **kwargs):
        act = action['act']
