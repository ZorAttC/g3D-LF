...
class SemanticGrid(object):
    ...
    def spatialTransformer(self, grid, pose, abs_pose):
        ...
        for j in range(grid.shape[0]): # sequence length
            ...
            rot_geo_grid = F.grid_sample(grid_step, rot_disp_grid.to(torch.float16), align_corners=False ) # apply rotation
            geo_grid = F.grid_sample(rot_geo_grid, trans_disp_grid.to(torch.float16), align_corners=False) # apply translation
