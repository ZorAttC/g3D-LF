...
class CrossmodalEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.grid_map_layers = nn.ModuleList(
            [GraphLXRTXLayer(config) for _ in range(self.num_x_layers)]
        )

    def forward(self, txt_embeds, txt_masks, img_embeds, img_masks, grid_map_fts, graph_sprels=None):
        extended_txt_masks = extend_neg_masks(txt_masks)
        extended_img_masks = extend_neg_masks(img_masks) # (N, 1(H), 1(L_q), L_v)
        
        for layer_id in range(len(self.x_layers)):
            img_embeds  += self.grid_map_layers[layer_id](
                grid_map_fts, None, 
                img_embeds, extended_img_masks,
                graph_sprels=graph_sprels
            )

            img_embeds = self.x_layers[layer_id](
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.occupancy_map_predictor = None
        self.semantic_map_predictor = None
        self.waypoint_predictor = None
        ...
        self.feature_fields = Feature_Fields(batch_size=1, device="cuda", mode='habitat',camera_intrinsic=None, bev=True) # Initialize the feature fields
    ...
    def forward_panorama(
        self, rgb_fts, loc_fts, nav_types, view_lens
    ):
    ...
    def forward_navigation(
        ...
        gmap_embeds = self.global_encoder.encoder(
            txt_embeds, txt_masks, gmap_embeds, gmap_masks, predicted_bev_fts,
            graph_sprels=graph_sprels
        )
        ...
        if mode == 'language':
            ...
            return pano_embeds, pano_masks
             ...
             return self.forward_navigation_per_step(
                ...
                batch['vp_nav_masks'], batch['vp_obj_masks'], batch['vp_cand_vpids'], batch['predicted_bev_fts']
            )