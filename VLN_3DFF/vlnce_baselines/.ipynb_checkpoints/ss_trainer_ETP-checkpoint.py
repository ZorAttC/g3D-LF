...
class RLTrainer(BaseVLNCETrainer):
    ...
    def _initialize_policy(
        ...
        self.policy.net.feature_fields.load_state_dict(torch.load("pretrained/3dff.pth"),strict=True)
        self.policy.net.feature_fields.eval()
        ...
        for stepk in range(self.max_len):
            ...
            if stepk == 0:
                ...
                if self.config.MODEL.task_type == 'r2r':
                    ...
                    policy_net.feature_fields.initialize_novel_view_setting(hfov=90., vfov=90.)
                elif self.config.MODEL.task_type == 'rxr':
                    policy_net.feature_fields.initialize_camera_setting(hfov=79., vfov=79.)
                    policy_net.feature_fields.initialize_novel_view_setting(hfov=79., vfov=79.)
            ...
            if sum(dones) > 0:
                for i in reversed(list(range(len(dones)))):
                    if dones[i]:
                        ...
                        policy_net.feature_fields.pop(i) # Remove the feature fields for i-th sample
